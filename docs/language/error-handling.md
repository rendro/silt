...
`Display` as a supertrait and provides `message(self) -> String`. String
interpolation `"{e}"` and `e.message()` produce the same text — silt routes
the `Display` impl through `Error::message` for stdlib error enums so the two
shapes never drift.
