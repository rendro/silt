...
adding a keyword (`if`, `for`, `while`, `mut`, `async`, `await`, `try`,
...
values defined in the prelude. `_` is a wildcard pattern token. `try` is a
builtin function. This keeps the keyword count honest.
...
locks. (2) Simpler reasoning -- values never change after creation. (3)
Implementation simplicity -- the interpreter uses `Rc<T>` with no `RefCell`
on user-facing values.
