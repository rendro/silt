...
pub fn call_result(vm: &mut Vm, name: &str, args: &[Value]) -> Result<Value, VmError> {
    match name {
        ...
        "is_ok" => {
            if args.len() != 1 {
                return Err(VmError::new("result.is_ok takes 1 argument".into()));
            }
            Ok(Value::Bool(
                matches!(&args[0], Value::Variant(tag, _) if tag == "Ok"),
            ))
        }
        "is_err" => {
            if args.len() != 1 {
                return Err(VmError::new("result.is_err takes 1 argument".into()));
            }
            Ok(Value::Bool(
                matches!(&args[0], Value::Variant(tag, _) if tag == "Err"),
            ))
        }
        "map_ok" => {
            if args.len() != 2 {
                return Err(VmError::new("result.map_ok takes 2 arguments".into()));
            }
            match &args[0] {
        ...
        "flat_map" => {
            if args.len() != 2 {
                return Err(VmError::new("result.flat_map takes 2 arguments".into()));
            }
            match &args[0] {
                Value::Variant(tag, fields) if tag == "Ok" && fields.len() == 1 => {
...
pub fn call_option(vm: &mut Vm, name: &str, args: &[Value]) -> Result<Value, VmError> {
    match name {
        ...
        "is_some" => {
            if args.len() != 1 {
                return Err(VmError::new("option.is_some takes 1 argument".into()));
            }
            Ok(Value::Bool(
                matches!(&args[0], Value::Variant(tag, _) if tag == "Some"),
            ))
        }
        "is_none" => {
            if args.len() != 1 {
                return Err(VmError::new("option.is_none takes 1 argument".into()));
            }
            Ok(Value::Bool(
                matches!(&args[0], Value::Variant(tag, _) if tag == "None"),
            ))
        }
        "to_result" => {
            if args.len() != 2 {
        ...
        "map" => {
            if args.len() != 2 {
                return Err(VmError::new("option.map takes 2 arguments".into()));
            }
            match &args[0] {
                Value::Variant(tag, fields) if tag == "Some" && fields.len() == 1 => {
                    let new_val = vm.invoke_callable(&args[1], &[fields[0].clone()])?;
        ...
        "flat_map" => {
            if args.len() != 2 {
                return Err(VmError::new("option.flat_map takes 2 arguments".into()));
            }
            match &args[0] {
                Value::Variant(tag, fields) if tag == "Some" && fields.len() == 1 => {
                    vm.invoke_callable(&args[1], &[fields[0].clone()])
