...
pub fn call_list(vm: &mut Vm, name: &str, args: &[Value]) -> Result<Value, VmError> {
    match name {
        "map" => {
            ...
            for item in xs.iter() {
                let val = vm.invoke_callable(func, &[item.clone()])?;
                result.push(val);
            }
        ...
        "filter" => {
            ...
            for item in xs.iter() {
                let keep = vm.invoke_callable(func, &[item.clone()])?;
        ...
        "each" => {
            ...
            let func = &args[1];
            for item in xs.iter() {
                vm.invoke_callable(func, &[item.clone()])?;
            }
            Ok(Value::Unit)
        }
        "fold" => {
        ...
        "find" => {
            ...
            let func = &args[1];
            for item in xs.iter() {
                let result = vm.invoke_callable(func, &[item.clone()])?;
        ...
        "any" => {
            ...
            for item in xs.iter() {
                let result = vm.invoke_callable(func, &[item.clone()])?;
                if vm.is_truthy(&result) { return Ok(Value::Bool(true)); }
            }
            Ok(Value::Bool(false))
        }
        "all" => {
            ...
            for item in xs.iter() {
                let result = vm.invoke_callable(func, &[item.clone()])?;
                if !vm.is_truthy(&result) { return Ok(Value::Bool(false)); }
            }
            Ok(Value::Bool(true))
        }
        "flat_map" => {
