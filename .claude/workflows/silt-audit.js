...
async function runRound(round) {
  ...
  const integration = await agent(
    ...
    `2b. LINT GATE (CI enforces this — a round that passes tests but fails lint still reddens the PR). After build+test is green, run \`cargo fmt\` (auto-format; patches routinely land unformatted) and \`cargo clippy --all-features -- -D warnings\`. Fix any clippy findings in the affected patch's files and re-run that finding's repro. Then confirm \`cargo fmt -- --check\` and \`cargo check --benches --all-features\` both pass. The commit must be fmt-clean and clippy-clean.\n` +
